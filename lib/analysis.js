...
class Analysis {
    constructor(goatsheep, parentCell = null) {
    ...
    calculateCells() {
        ...
        const bounds = this.parent ? parent.bounds : { top: 0, left: 0, width: this.goatsheep.width, height: this.goatsheep.height };
        // Calculate cell size
        let sizeX = Math.max(...goatsheep.objects.map(o => o.width));
        let sizeY = Math.max(...goatsheep.objects.map(o => o.height));
        ...
        let yPositions = (new Array(yPositionCount)).map((_, i) => bounds.top + i * cellHeight);
        ...
        for (let posX of xPositions) {
            for (let posY of yPositions) {
                cells.push(new Cell(posX, posY, cellWidth, cellHeight, goatsheep, parentCell));
            }
        }
        // TODO: Score cells
    }
